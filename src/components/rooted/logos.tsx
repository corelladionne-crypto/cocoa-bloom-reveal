...
export function ProjectLogos({ className = "" }: { className?: string }) {
  return (
    <div className={`flex items-center gap-4 ${className}`}>
      <BrandMark src={cadburyLogo} label="Cadbury" className="h-10 w-32" />
      <BrandMark src={asuLogo} label="Arizona State University" className="h-10 w-28" />
      <BrandMark src={cfLogo} label="Changing Futures" className="h-10 w-32" />
    </div>
  );
}
